...
- **One exception: an interactive login when publishing.** See `references/publishing.md`.
...
| Empty/near-empty folder, wants a docs site | Scaffold — `references/scaffolding.md` |
...
**Don't wait on that line alone** — some shells buffer output until the process exits, which a server never does. Use the **single command** in `references/preview.md`. It picks a free port, starts the server in the background, and waits for either the JSON line or the port answering, all in one shell call (it allows up to a minute for a first-run `npx` download). Don't split it into separate checks, and don't write your own wait. If it prints the server's error output instead of a URL, that's the Step 2 failure path.
...
Subfolder: `npx --yes seemore docs`. Ports, LAN serving, one-server-per-project: `references/preview.md`.

## Step 4. Write and edit content

Read `references/content-authoring.md` before writing anything non-trivial. The parts that bite most often:
...
Build errors are content errors and name the file: duplicate address, unknown component, invalid frontmatter, dead link. Fix and rebuild — details in `references/troubleshooting.md`.
...
Offer once a build succeeds. Read `references/publishing.md` and **let the user pick the host** — GitHub Pages, Netlify, Cloudflare Pages, Surge, Vercel.
...
Publishing is the one place the user may need to act: some hosts need a one-time interactive login, and GitHub Pages needs GitHub Actions selected in repo settings. `references/publishing.md` covers handing those off cleanly and taking the work back.
...
- `references/scaffolding.md` — starting from nothing (Step 1)
- `references/preview.md` — dev server, ports (Step 3)
- `references/content-authoring.md` — Markdown/MDX syntax, components, ordering, page addresses (Step 4)
- `references/configuration.md` — every config option, themes, feature flags, search (Step 5)
- `references/publishing.md` — building and deploying to a live URL (Steps 6–7)
- `references/troubleshooting.md` — the errors that actually come up, and their fixes (any step)
